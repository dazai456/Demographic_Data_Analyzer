...
def calculate_demographic_data(print_data=True):
    ...
    filtered = ((df[top_salary].groupby('native-country')['salary'].count() \
      / df['native-country'].value_counts()) * 100).sort_values(ascending=False)
    highest_earning_country = filtered.index[0]
    highest_earning_country_percentage = filtered.iloc[0].round(1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df[(df['native-country'] == 'India') & top_salary]['occupation'].value_counts().index[0]
